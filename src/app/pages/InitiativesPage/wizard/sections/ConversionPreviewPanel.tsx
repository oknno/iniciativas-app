    ...
    <div style={{ display: 'grid', gap: tokens.spacing.sm }}>
      {groups.map((group) => (
        <div key={group.conversionCode} style={{ border: `1px solid ${tokens.colors.border}`, borderRadius: tokens.radius.md }}>
          <div style={{ padding: `${tokens.spacing.xs}px ${tokens.spacing.md}px`, borderBottom: `1px solid ${tokens.colors.border}`, background: '#f8fafc' }}>
          ...
          >
            {MONTHS.map(({ month, label }) => (
              <div key={month} style={{ minWidth: 64 }}>
